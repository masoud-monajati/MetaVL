...
def get_params_for_weight_decay_optimization(module, config):
    ...
    #print("module.named_parameters()",module.named_parameters())
    #print(begh)
    
    for module_ in module.modules():
        #print("len(module_.named_parameters())",len(list(module_.named_parameters())))
        #print("len(module_.named_parameters())",list(module_.named_parameters()))
        #print("len(module_.named_parameters())",list(module_))
        #print(begh)
        if isinstance(module_, blacklist_modules) or (
            config.weight_decay == 0.0
        ):  # also include all parameters here if no weight decay is being done
            '''
            for n,p in list(module_.named_parameters()):
                if (p is not None) and p.requires_grad:
                    #print(n)
                    no_weight_decay_params["params"].extend([p])
                    if n in no_w_list:
                        print(n)
                        print(begh)
                    no_w_list.append(n)
    ...
    '''        
    print('param_keys',param_dict.keys())
    print('----------------------------')
    print('no_w_list',no_w_list)
    print('----------------------------')
    print('param_dict',len(param_dict.keys()))
    print('----------------------------')
    print('len_no_weight_param',len(no_weight_decay_params["params"]))
    print('----------------------------')
    print('param_list',len(param_list))
    print('----------------------------')
    print('no_w_list',len(no_w_list))
    #print('no_weight_param',weight_decay_params["params"])
    print('----------------------------')
    '''
    if len(param_dict.keys()) != len(no_weight_decay_params["params"]):
        no_weight_decay_params["params"]=no_weight_decay_params["params"][:-1]
        print('len_no_weight_param',len(no_weight_decay_params["params"]))
        #print(begh)
    print('len_no_weight_param1',len(no_weight_decay_params["params"]))
    assert len(no_weight_decay_params["params"]) + len(   weight_decay_params["params"]
    ) == len(
        param_dict.keys()
...
def configure_param_groups(model, config):
    ...
    #print("len(all_params)",all_params[1].keys())
    #print(begh)
    #print("len(all_params)",len(all_params[1]["params"]))
    ...
    #print("d0.keys()",len(d['lr_2e-06_wd_0.0']["params"]))
    #print("d1.keys()",len(d['lr_None_wd_0.0']["params"]))
...
def build_labels(
    ...
    for label in labels:
        for k, token in enumerate(label):
            if token == 24564:
                #print("k2",k)
                #print("DDDD",label[k-3:k+12])
                label[:k+5] = -100
                break 
            #elif 
    
    #print("labels",labels[0,145:165])
    #print("labels",labels[1,145:165])
    #print(begh)
    '''
