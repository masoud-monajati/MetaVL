...
def get_pretraining_datasets(config, tokenizer, transforms):
    ...
    eval_len = int(len(train_dataset) * 0.5)
    train_len = len(train_dataset) - eval_len
    train_dataset1, train_dataset2 = random_split(train_dataset, [train_len, eval_len])
    
    print_main(f"Loaded train dataset with {len(train_dataset1)} samples")
    ...
    return train_dataset1, eval_dataset 
